...
const series = [
  { key: 'monthly_income', label: 'Income', color: '#2563eb', kind: 'bar' },
  { key: 'monthly_expenses', label: 'Expenses', color: '#e11d48', kind: 'bar' },
  { key: 'automated_savings', label: 'Automated savings', color: '#059669', kind: 'bar' },
  { key: 'spending_net_cash_flow', label: 'Spending cash flow', color: '#7c3aed', kind: 'line' },
  { key: 'monthly_savings', label: 'Monthly savings', color: '#0f766e', kind: 'line' },
  { key: 'savings_rate', label: 'Savings rate', color: '#d97706', kind: 'rate' },
] as const;
...
export function HistoricalTrends() {
  ...
  return <section className="card mt-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Historical trends</h2><p className="mt-1 text-sm text-slate-500">Monthly cash-flow and savings movement. Click a month for account sources.</p></div><label className="text-sm text-slate-600">Period <select className="ml-2 rounded border p-2" value={months} onChange={(event) => { setMonths(Number(event.target.value)); setSelected(null); }}><option value={3}>3 months</option><option value={6}>6 months</option><option value={12}>12 months</option></select></label></div>
    ...
    {points.length ? <div className="mt-5 h-80"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={points} margin={{ top: 12, right: 12, left: -12, bottom: 0 }} onClick={(chart: any) => { const point=chart?.activePayload?.[0]?.payload as TrendPoint | undefined; if (point) setSelected(point); }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0"/><XAxis dataKey="label" tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false}/><YAxis yAxisId="money" tickFormatter={(value) => `$${Math.round(value / 1000)}k`} tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false}/><YAxis yAxisId="rate" orientation="right" tickFormatter={(value) => `${value}%`} tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false}/><Tooltip content={<TrendTooltip />}/>{visible.monthly_income && <Bar yAxisId="money" dataKey="monthly_income" fill="#2563eb" radius={[4,4,0,0]} maxBarSize={28}/>} {visible.monthly_expenses && <Bar yAxisId="money" dataKey="monthly_expenses" fill="#e11d48" radius={[4,4,0,0]} maxBarSize={28}/>} {visible.automated_savings && <Bar yAxisId="money" dataKey="automated_savings" fill="#059669" radius={[4,4,0,0]} maxBarSize={28}/>} {visible.spending_net_cash_flow && <Line yAxisId="money" type="monotone" dataKey="spending_net_cash_flow" stroke="#7c3aed" strokeWidth={2} dot={{ r: 3 }}/>} {visible.monthly_savings && <Line yAxisId="money" type="monotone" dataKey="monthly_savings" stroke="#0f766e" strokeWidth={3} dot={{ r: 3 }}/>} {visible.savings_rate && <Line yAxisId="rate" type="monotone" dataKey="savings_rate" stroke="#d97706" strokeWidth={2} strokeDasharray="4 3" dot={{ r: 3 }}/>}</ComposedChart></ResponsiveContainer></div> : <p className="mt-6 text-sm text-slate-500">No transaction history is available for this period.</p>}

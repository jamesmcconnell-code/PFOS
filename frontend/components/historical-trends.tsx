...
function TrendTooltip({ active, payload }: any) {
  ...
  return <div className="min-w-52 rounded-xl border border-slate-200 bg-white p-3 shadow-lg"><p className="font-semibold">{point.label}</p><div className="mt-2 space-y-1 text-xs">{series.map((item) => <div className="flex justify-between gap-4" key={item.key}><span className="text-slate-600">{item.label}</span><b className={status(Number(point[item.key]))}><FinancialValue>{item.kind === 'rate' ? `${point.savings_rate}%` : money(point[item.key])}</FinancialValue></b></div>)}</div><p className="mt-2 text-[11px] text-slate-400">Click this month for account sources.</p></div>;
}

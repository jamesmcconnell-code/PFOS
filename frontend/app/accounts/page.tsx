    ...
    try {
      await api(`/accounts/${selected.id}`, {
        ...
        body: JSON.stringify({
          ...
          is_savings_direct_deposit: selected.is_savings_direct_deposit === true || selected.is_savings_direct_deposit === 'true',
        }),
      });
      setSelected(null);
      await load();
  ...
  return <Protected>
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="account-editor">
      <form onSubmit={save} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <label className="mt-4 flex items-center gap-2 text-sm"><input type="checkbox" checked={selected.is_savings_direct_deposit === true || selected.is_savings_direct_deposit === 'true'} onChange={(event) => setSelected({ ...selected, is_savings_direct_deposit: event.target.checked })} /> Savings Direct Deposit account</label>

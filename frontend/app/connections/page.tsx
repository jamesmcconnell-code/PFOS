...
export default function ConnectedSources() {
  ...
  const load = () => api('/connections').then(setRows);
  useEffect(() => { void load(); }, []);
  ...
  return <Protected>
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-2">
      ...
      <div className="card"><h2 className="font-semibold">Plaid bank connection</h2><button onClick={launchPlaid} className="mt-4 rounded bg-navy px-4 py-2 text-white">Connect a bank with Plaid</button></div>
    </section>
    <section className="mt-6"><h2 className="font-semibold">Connected sources</h2>
      <div className="mt-3 space-y-3">{rows.map((connection) => <div className="card flex items-center justify-between gap-4" key={connection.id}>
        <button onClick={() => { setSelected(connection); setConfirmingDelete(false); }} className="min-w-0 text-left"><b className="capitalize">{connection.provider}</b><p className="truncate text-sm text-slate-500">{connection.name} · {connection.last_synced_at ? `Synced ${connection.last_synced_at}` : 'Not synced yet'}</p></button>
        <div className="flex shrink-0 gap-2">{connection.provider !== 'csv' && <button onClick={() => void sync(connection)} disabled={syncing === connection.id} className="rounded border px-3 py-2 text-sm disabled:opacity-60">{syncing === connection.id ? 'Syncing…' : 'Sync'}</button>}<button onClick={() => { setSelected(connection); setConfirmingDelete(false); }} className="rounded border px-3 py-2 text-sm">Manage</button></div>
      </div>)}</div>
    </section>

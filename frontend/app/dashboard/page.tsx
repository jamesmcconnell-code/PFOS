...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const handleViewChange=()=>{load();updateScopeLabel();};
    load();
    updateScopeLabel();
    const refresh = window.setInterval(load, 30000);
    window.addEventListener('pfos-view-change', handleViewChange);
    window.addEventListener('focus', load);
    return () => { window.clearInterval(refresh); window.removeEventListener('pfos-view-change', handleViewChange); window.removeEventListener('focus', load); };
  }, [period]);

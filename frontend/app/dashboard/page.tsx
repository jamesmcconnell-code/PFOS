...
function MonthMetric({ label, value, kind = 'positive' }: { label: string; value: number; kind?: 'positive' | 'expense' | 'rate' }) {
  ...
  return <div className={`flex items-center justify-between gap-3 rounded-lg px-2 py-2 ${tone}`}>
    <dt>{label}</dt><dd className="flex items-center gap-2 font-medium"><span className="hidden text-[11px] sm:inline">{status}</span>{kind === 'rate' ? `${value}%` : money(value)}</dd>
  </div>;
}

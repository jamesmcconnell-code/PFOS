...
export default function Transactions() {
  ...
  const [transactionType, setTransactionType] = useState(''), [sort, setSort] = useState('date_desc'), [page, setPage] = useState(1), [total, setTotal] = useState(0), [totalPages, setTotalPages] = useState(1), [error, setError] = useState('');
  ...
  }, [search, page, sort, categoryIds, financialRoles, connectionIds, transactionType]);
  ...
  return <Protected><h1 className="text-3xl font-bold">Transactions</h1><p className="mt-1 text-slate-500">Search and filter your transaction history without moving data into the browser.</p>
    <section className="card mt-5"><input className="w-full rounded-lg border bg-white p-3" placeholder="Search description…" value={search} onChange={(event) => resetPage(setSearch, event.target.value)}/><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">

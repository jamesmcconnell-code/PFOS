...
def main():
 db=SessionLocal()
 if db.query(User).filter_by(email='james@example.com').first(): return
 u=User(email='james@example.com',display_name='James',password_hash=hash_password('change-me-now')); h=Household(name='James & Fiancee');db.add_all([u,h]);db.flush();db.add(HouseholdMember(household_id=h.id,user_id=u.id,role='owner'))
 ...
 db.commit(); print('Seeded james@example.com / change-me-now')

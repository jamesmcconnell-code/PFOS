...
def upgrade():
    with op.batch_alter_table('transaction_classification_suggestions') as batch_op:
        batch_op.add_column(sa.Column('proposal_data', sa.Text(), nullable=True))
        batch_op.add_column(sa.Column('evidence_data', sa.Text(), nullable=True))
        batch_op.add_column(sa.Column('recurring', sa.Boolean(), nullable=True))
        batch_op.add_column(sa.Column('source_type', sa.String(20), nullable=False, server_default='rule'))
    op.create_table('transaction_classification_suggestion_decisions', sa.Column('id', sa.Uuid(), primary_key=True), sa.Column('suggestion_id', sa.Uuid(), sa.ForeignKey('transaction_classification_suggestions.id', ondelete='CASCADE'), nullable=False), sa.Column('field_name', sa.String(40), nullable=False), sa.Column('decision', sa.String(20), nullable=False), sa.Column('value_data', sa.Text(), nullable=True), sa.Column('created_at', sa.DateTime(), nullable=False), sa.Column('updated_at', sa.DateTime(), nullable=False), sa.UniqueConstraint('suggestion_id', 'field_name', name='uq_suggestion_decision_field'))
...
def downgrade():
    ...
    with op.batch_alter_table('transaction_classification_suggestions') as batch_op:
        ...
        batch_op.drop_column('evidence_data')

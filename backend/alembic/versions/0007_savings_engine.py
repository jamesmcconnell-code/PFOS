...
def upgrade():
    op.add_column('accounts',sa.Column('is_savings_direct_deposit',sa.Boolean(),nullable=False,server_default=sa.false()))
    ...
    op.alter_column('accounts','is_savings_direct_deposit',server_default=None);op.alter_column('transactions','is_internal_transfer',server_default=None);op.alter_column('savings_rules','is_active',server_default=None)
def downgrade():
    op.drop_table('savings_rules');op.drop_column('transactions','is_internal_transfer');op.drop_column('accounts','is_savings_direct_deposit')
